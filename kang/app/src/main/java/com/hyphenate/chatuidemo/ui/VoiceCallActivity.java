...
public class VoiceCallActivity extends CallActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        final TextView nickTextView = (TextView) findViewById(R.id.tv_nick);//昵称
        final ImageView headCard = (ImageView) findViewById(R.id.swing_card);//头像
        ...
        if (DemoHelper.getInstance().isContainsKey(username)){
            UserInfoBean bean = DemoHelper.getInstance().getUserBeanForUid(username);
            if (bean != null){
            ...
            DemoHelper.getInstance().getUserBean(username, new DemoHelper.UserInfoListener() {
                ...
                public void succeed(UserInfoBean bean) {
                    if (bean != null){
                        nickTextView.setText(bean.getNickname());
                        ImageHelper.load(VoiceCallActivity.this,bean.getPic(),headCard,null,true,R.drawable.em_default_avatar,R.drawable.em_default_avatar);
                    }
                }
            });
        }
